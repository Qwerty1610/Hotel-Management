...
import com.mycompany.hotelmanagement.service.DashboardService.DashboardQuery;

/**
 * ManagerDashboardController
 * Trang Tổng quan của Manager, thiết kế đồng bộ với Bảng điều khiển hệ thống
 * của Admin (UC 2.7.4):
 * - Một kỳ báo cáo chung cho các thẻ KPI (tháng / quý / năm / tùy chỉnh,
 *   tiền tố tham số {@code kp}).
 * - Mỗi biểu đồ có khoảng lọc riêng (tiền tố {@code c1}..{@code c4}).
 * - Bấm thẻ "Tổng doanh thu" / "Tỷ lệ hủy" mở danh sách chi tiết render
 *   ở backend, phân trang ({@code view} + {@code page}).
 *
 * Quyền truy cập (role MANAGER) được kiểm soát bởi AuthFilter.
 *
 * Date: 22/07/2026
 * version 1.2
...
public class ManagerDashboardController extends HttpServlet {
    ...
    /** Kỳ lọc đã phân giải: giá trị echo cho form + khoảng ngày để truy vấn. */
    ...
    /** Tải số liệu cho tab Tổng quan: KPI theo kỳ báo cáo + 4 biểu đồ + danh sách chi tiết. */
    private void loadOverview(HttpServletRequest request) {
        ...
        ResolvedPeriod kp = resolvePeriod(request, "kp", today);
        q.kpiFrom = kp.from;
        q.kpiTo = kp.to;
        ...
        LocalDate[] c3 = resolveRange(request, "c3", today.minusDays(89), today);
        ...
        DashboardStats stats = dashboardService.getStats(q);
        stats.setKpiPeriod(kp.period);

        // Danh sách năm / quý cho ô chọn của bộ lọc kỳ
        ...
        request.setAttribute("baseQuery", buildBaseQuery(kp, c1, c2, c3, c4));
    ...
    /**
     * Đọc bộ lọc kỳ theo tiền tố ({@code kp}): {@code <p>Mode} = month | quarter
     * | year | custom cùng giá trị tương ứng {@code <p>Month} (yyyy-MM),
     * {@code <p>Quarter} (yyyy-Qn), {@code <p>Year}, {@code <p>From}/{@code <p>To}.
     * Giá trị thiếu / sai định dạng rơi về mặc định (tháng hiện tại).
     */
    ...
    private String whitelistView(String view) {
        if ("revenue".equals(view) || "cancelled".equals(view)) {
            return view;
        }
        return null;
    }
     ...
     */
    private String buildBaseQuery(ResolvedPeriod kp,
                                  LocalDate[] c1, LocalDate[] c2, LocalDate[] c3, LocalDate[] c4) {
        StringBuilder sb = new StringBuilder();
        appendPeriod(sb, "kp", kp.period);
