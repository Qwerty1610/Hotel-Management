        ...
        <div class="dashboard-main">
            ...
            <main class="workspace-content">
                ...
                <div class="table-card" style="margin-bottom: 24px;">
                    ...
                    <form class="table-filter-bar" method="get" action="${pageContext.request.contextPath}/manager/requests"
                        ...
                        <div class="modal-form-group" style="margin-bottom:0;">
                            ...
                            <div class="search-wrapper" style="max-width:100%;">
                                ...
                                <input type="text" name="q" class="input-search-service" placeholder="Nhập số phòng..." value="<c:out value='${q}' />" />
                            </div>
                        </div>
                        <div class="modal-form-group" style="margin-bottom:0;">
                            <label>Mức độ ưu tiên</label>
                            <select name="priority" class="status-select" onchange="this.form.submit()" style="width:100%;">
                                <option value="all"    ${priorityFilter eq 'all'    ? 'selected' : ''}>Tất cả mức độ</option>
                                <option value="Urgent" ${priorityFilter eq 'Urgent' ? 'selected' : ''}>Khẩn cấp</option>
                        ...
                        <div class="modal-form-group" style="margin-bottom:0;">
                            ...
                            <select name="staff" class="status-select" onchange="this.form.submit()" style="width:100%;">
                                <option value="all"        ${staffFilterVal eq 'all'        ? 'selected' : ''}>Tất cả nhân viên</option>
                                <option value="unassigned" ${staffFilterVal eq 'unassigned' ? 'selected' : ''}>Chưa gán</option>
                                <c:forEach var="s" items="${staffList}">
                                    <c:set var="sidStr">${s.accountId}</c:set>
                                    <option value="${s.accountId}" ${staffFilterVal eq sidStr ? 'selected' : ''}><c:out value="${s.fullName}" /></option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="modal-form-group" style="margin-bottom:0;">
                            ...
                            <select name="status" class="status-select" onchange="this.form.submit()" style="width:100%;">
                                ...
                                <option value="InProgress" ${statusFilter eq 'InProgress' ? 'selected' : ''}>Đang thực hiện</option>
                        ...
                        <button type="submit" class="btn-add-service" style="height:40px;"><i class="fa-solid fa-magnifying-glass"></i> Lọc</button>
                    </form>
                                ...
                                <tr>
                                    <td>
                                        <div class="service-name-cell"><div>
                                            <span class="service-title"><c:out value="${r.title}" /></span>
                                            <span class="request-sub">
                                                <i class="fa-solid fa-bed"></i> Phòng <c:out value="${r.roomNumber}" />
                                                &nbsp;•&nbsp; <i class="fa-regular fa-clock"></i> <fmt:formatDate value="${r.createdAt}" pattern="dd/MM/yyyy HH:mm" />
                                            </span>
                                        </div></div>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${r.priority eq 'Urgent'}"><span class="prio-badge prio-urgent">KHẨN CẤP</span></c:when>
                                            <c:when test="${r.priority eq 'High'}"><span class="prio-badge prio-high">CAO</span></c:when>
                                            <c:when test="${r.priority eq 'Low'}"><span class="prio-badge prio-low">THẤP</span></c:when>
                                            <c:otherwise><span class="prio-badge prio-medium">TRUNG BÌNH</span></c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${not empty r.assignedStaffName}"><span style="font-weight:600; color:var(--text-navy);"><c:out value="${r.assignedStaffName}" /></span></c:when>
                                            <c:otherwise><span style="color:var(--text-muted); font-style:italic;">Chưa gán</span></c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${r.status eq 'InProgress'}"><span class="status-pill status-occupied"><i class="fa-solid fa-circle"></i> ĐANG THỰC HIỆN</span></c:when>
                                            <c:when test="${r.status eq 'Completed'}"><span class="status-pill status-available"><i class="fa-solid fa-circle"></i> HOÀN THÀNH</span></c:when>
                                            <c:when test="${r.status eq 'Cancelled'}"><span class="status-pill status-maintenance"><i class="fa-solid fa-circle"></i> ĐÃ HUỶ</span></c:when>
                                            <c:otherwise><span class="status-pill status-cleaning"><i class="fa-solid fa-circle"></i> ĐANG CHỜ</span></c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <div class="table-actions" style="display:flex; gap:8px;">
                                            <c:choose>
                                                <c:when test="${r.status eq 'Pending' or r.status eq 'InProgress'}">
                                                    <button class="btn-action assign" title="${r.assignedStaffId != null ? 'Đổi NV' : 'Gán việc'}"
                                                            data-title="<c:out value='${r.title}' />" data-room="<c:out value='${r.roomNumber}' />"
                                                            onclick="openAssignModal(${r.requestId}, this)"><i class="fa-solid fa-user-plus"></i></button>
                                                    <c:choose>
                                                        <c:when test="${r.assignedStaffId != null}">
                                                            <button class="btn-action done" title="Hoàn thành" onclick="changeStatus(${r.requestId}, 'Completed')"><i class="fa-solid fa-check"></i></button>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <button class="btn-action done" title="Cần gán nhân viên trước khi hoàn thành" disabled style="opacity:.35; cursor:not-allowed;"><i class="fa-solid fa-check"></i></button>
                                                        </c:otherwise>
                                                    </c:choose>
                                                    <button class="btn-action delete" title="Huỷ" onclick="changeStatus(${r.requestId}, 'Cancelled')"><i class="fa-solid fa-xmark"></i></button>
                                                </c:when>
                                                <c:otherwise><span style="color:var(--text-muted); font-size:12px;">—</span></c:otherwise>
                                            </c:choose>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div class="table-pagination-bar">
                        <div class="pagination-info">
                            <c:choose>
                                ...
                                <c:otherwise>Hiển thị ${(page-1)*pageSize + 1}-${page*pageSize gt totalItems ? totalItems : page*pageSize} trong số ${totalItems} yêu cầu</c:otherwise>
                            </c:choose>
                        </div>
                        <div class="pagination-controls">
                            <c:choose>
                                <c:when test="${page > 1}">
                                    <c:url var="prevUrl" value="/manager/requests"><c:param name="q" value="${q}"/><c:param name="priority" value="${priorityFilter}"/><c:param name="staff" value="${staffFilterVal}"/><c:param name="status" value="${statusFilter}"/><c:param name="page" value="${page-1}"/></c:url>
                            ...
                            <c:forEach var="p" begin="1" end="${totalPages}">
                                <c:url var="pUrl" value="/manager/requests"><c:param name="q" value="${q}"/><c:param name="priority" value="${priorityFilter}"/><c:param name="staff" value="${staffFilterVal}"/><c:param name="status" value="${statusFilter}"/><c:param name="page" value="${p}"/></c:url>
                                <a class="btn-page ${p == page ? 'active' : ''}" href="${pUrl}">${p}</a>
                            </c:forEach>
                            <c:choose>
                                <c:when test="${page < totalPages}">
                                    <c:url var="nextUrl" value="/manager/requests"><c:param name="q" value="${q}"/><c:param name="priority" value="${priorityFilter}"/><c:param name="staff" value="${staffFilterVal}"/><c:param name="status" value="${statusFilter}"/><c:param name="page" value="${page+1}"/></c:url>
                ...
                <div class="content-header-row" style="margin-bottom:16px;">
                    <div>
                        ...
                        <p>Trạng thái trực và số công việc đã hoàn thành của nhân viên buồng phòng. Bấm vào một dòng để xem chi tiết.</p>
                    </div>
                </div>
                ...
                <div class="table-card">
                    <table class="services-table-element">
                        ...
                        <tbody>
                            <c:forEach var="s" items="${staffList}">
                                <tr style="cursor:pointer;" onclick="window.location.href='${pageContext.request.contextPath}/manager/staff?id=${s.accountId}'">
                                    <td>
                                        <div class="service-name-cell">
                                            <div class="staff-avatar-sm"><i class="fa-solid fa-user"></i></div>
                                            <div><span class="service-title"><c:out value="${s.fullName}" /></span>
                                                <span class="request-sub"><c:out value="${s.email}" /></span></div>
                                        </div>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${s.workStatus eq 'Active'}"><span class="wk-badge wk-active"><i class="fa-solid fa-circle"></i> Đang trực</span></c:when>
                                            <c:when test="${s.workStatus eq 'OnBreak'}"><span class="wk-badge wk-break"><i class="fa-solid fa-circle"></i> Đang nghỉ</span></c:when>
                                    ...
                                    <td><span style="font-weight:700; color:var(--brand-blue);">${s.completedToday}</span></td>
                                    <td><span style="font-weight:600;">${s.completedMonth} việc</span></td>
                                </tr>
                            </c:forEach>
                            <c:if test="${empty staffList}">
                                <tr><td colspan="5" style="text-align:center; padding:40px; color:var(--text-muted);">Chưa có nhân viên buồng phòng</td></tr>
                            </c:if>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="modal-body">
                ...
                <form id="assignForm" action="${pageContext.request.contextPath}/manager/requests" method="post">
                    ...
                    <div class="modal-form-group">
                        ...
                        <small id="noActiveStaffHint" style="display:none; color:#dc2626; font-weight:600;">Hiện không có nhân viên nào đang trực.</small>
                    </div>
