        ...
        <div class="receipt-layout">
            ...
            <div class="booking-card" style="padding: 40px; position: relative;">
                ...
                <div class="receipt-header">
                    ...
                    <p style="color: var(--text-muted); font-size: 14px; margin-top: 5px;">Mã đặt phòng: #${booking.bookingId}</p>
                    
                    <div style="margin-top: 15px;">
                        <c:choose>
                            <c:when test="${booking.status eq 'Pending'}">
                                <span class="badge badge-pending" style="font-size: 14px; padding: 6px 14px;">Chờ duyệt</span>
                            </c:when>
                            <c:when test="${booking.status eq 'Confirmed'}">
                                <span class="badge badge-confirmed" style="font-size: 14px; padding: 6px 14px;">Đã xác nhận</span>
                            </c:when>
                            <c:when test="${booking.status eq 'CheckedIn'}">
                                <span class="badge badge-checkedin" style="font-size: 14px; padding: 6px 14px;">Đã nhận phòng</span>
                            </c:when>
                            <c:when test="${booking.status eq 'CheckedOut'}">
                                <span class="badge badge-checkedout" style="font-size: 14px; padding: 6px 14px;">Đã trả phòng</span>
                            </c:when>
                            <c:when test="${booking.status eq 'Cancelled'}">
                                <span class="badge badge-cancelled" style="font-size: 14px; padding: 6px 14px;">Đã hủy</span>
                            </c:when>
                            <c:when test="${booking.status eq 'Rejected'}">
                                <span class="badge badge-rejected" style="font-size: 14px; padding: 6px 14px;">Từ chối</span>
                            </c:when>
                        </c:choose>
                    </div>
                </div>
                ...
                <div class="receipt-grid-2">
                    <div class="receipt-info-section">
                        ...
                        <div class="receipt-info-row">
                            <span>Tài khoản đặt:</span>
                            <span>${sessionScope.email}</span>
                        </div>
                        <div class="receipt-info-row">
                            <span>Ngày lập đơn:</span>
                            <span><fmt:formatDate value="${booking.createdAt}" pattern="dd/MM/yyyy" /></span>
                        </div>
                    </div>
                    ...
                    <div class="receipt-info-section">
                        ...
                        <div class="receipt-info-row">
                            ...
                            <span><fmt:formatDate value="${booking.checkInDate}" pattern="dd/MM/yyyy" /></span>
                        </div>
                        <div class="receipt-info-row">
                            <span>Ngày trả phòng:</span>
                            <span><fmt:formatDate value="${booking.checkOutDate}" pattern="dd/MM/yyyy" /></span>
                        </div>
                        <div class="receipt-info-row">
                ...
                <h3 style="font-size: 16px; color: var(--primary-indigo); margin: 30px 0 15px 0; border-bottom: 1px solid var(--border-color); padding-bottom: 5px;">
                        ...
                        <c:choose>
                                ...
                                <c:forEach var="r" items="${rooms}" varStatus="status">
                                    <tr>
                                        ...
                                        <td style="font-style: italic;">
                                            <c:out value="${not empty r.guestName ? r.guestName : 'N/A'}" />
                                        </td>
                                    </tr>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
                                <tr>
                                    ...
                                    <td style="font-style: italic;">${booking.customerName} (Theo đơn đặt)</td>
                                </tr>
                            </c:otherwise>
                        </c:choose>
                    </tbody>
                ...
                <c:if test="${not empty booking.note}">
                    <div style="background-color: #fffbeb; border: 1px solid #fef3c7; border-radius: 8px; padding: 15px; margin-bottom: 30px;">
                        <h4 style="margin: 0 0 5px 0; font-size: 14px; color: #b45309;"><i class="fa-solid fa-comment-dots"></i> Ghi chú / Yêu cầu đặc biệt:</h4>
                        <p style="margin: 0; font-size: 13.5px; color: #78350f; line-height: 1.5;">${booking.note}</p>
                    </div>
                </c:if>

                <%-- Pricing breakdown --%>
                <div style="border-top: 2px dashed var(--border-color); padding-top: 20px; width: 300px; margin-left: auto;">
                    <div class="receipt-info-row" style="margin-bottom: 12px;">
                        <span>Tổng tiền phòng:</span>
                        <span style="font-weight: 600; font-size: 15px;">
                            <fmt:formatNumber value="${booking.totalAmount}" type="currency" currencySymbol="" /> VND
                        </span>
                    </div>
                    <div class="receipt-info-row" style="margin-bottom: 12px; border-bottom: 1px solid var(--border-color); padding-bottom: 10px;">
                        <span>Tiền đặt cọc (30%):</span>
                        <span style="font-weight: 600; color: var(--accent-gold); font-size: 15px;">
                            <fmt:formatNumber value="${booking.totalAmount * 0.3}" type="currency" currencySymbol="" /> VND
                        </span>
                    </div>
                    <div class="receipt-info-row" style="font-size: 18px; font-weight: 700; color: var(--primary-dark);">
                        <span>TỔNG TIỀN:</span>
                        <span><fmt:formatNumber value="${booking.totalAmount}" type="currency" currencySymbol="" /> VND</span>
                    </div>
                </div>

                <%-- Controls --%>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 40px; border-top: 1px solid var(--border-color); padding-top: 20px;">
                    ...
                    <c:if test="${booking.status eq 'Pending' || booking.status eq 'Confirmed'}">
                        <button type="button" class="btn-danger" style="padding: 10px 20px;" onclick="confirmCancel(${booking.bookingId})">
