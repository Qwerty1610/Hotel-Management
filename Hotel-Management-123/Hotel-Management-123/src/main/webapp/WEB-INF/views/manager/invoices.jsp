                        ...
                        <main class="workspace-content">
                            ...
                            <div class="stat-grid" style="grid-template-columns: repeat(2, 1fr);">
                                <div class="stat-card">
                                    ...
                                    <div class="stat-body">
                                        ...
                                        <span class="stat-value">
                                            <fmt:formatNumber value="${unpaidTotal}" type="number"
                                ...
                                <div class="stat-card">
                                    ...
                                    <div class="stat-body">
                                        ...
                                        <span class="stat-value">
                                            <fmt:formatNumber value="${refundingTotal}" type="number"
                                                maxFractionDigits="0" /> đ
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="table-card">
                                ...
                                <form class="table-filter-bar" method="get"
                                    action="${pageContext.request.contextPath}/manager/invoices">
                                    ...
                                    <select name="status" class="status-select" onchange="this.form.submit()">
                                        <option value="all" ${statusFilter eq 'all' ? 'selected' : '' }>Tất cả trạng
                                            thái</option>
                                        <option value="Pending" ${statusFilter eq 'Pending' ? 'selected' : '' }>Chưa
                                            thanh toán</option>
                                        <option value="Paid" ${statusFilter eq 'Paid' ? 'selected' : '' }>Đã thanh toán
                                        </option>
                                        <option value="Refunding" ${statusFilter eq 'Refunding' ? 'selected' : '' }>Chờ
                                            hoàn</option>
                                ...
                                <table class="services-table-element">
                                    ...
                                    <tbody>
                                        <c:forEach var="inv" items="${invoices}">
                                            <tr>
                                                <td>
                                                    <span style="font-weight:700; color:var(--brand-blue);">HD-
                                                        <fmt:formatNumber value="${inv.invoiceId}" minIntegerDigits="4"
                                                            groupingUsed="false" />
                                                    </span>
                                                </td>
                                                <td>
                                                    <div class="service-name-cell">
                                                        <div>
                                                            ...
                                                            <span class="request-sub"><i class="fa-solid fa-bed"></i>
                                                                Phòng
                                                ...
                                                <td><span style="color:#475569;">
                                                        <fmt:formatDate value="${inv.createdAt}"
                                                            pattern="dd/MM/yyyy HH:mm" />
                                                    </span></td>
                                                <td><span style="font-weight:700; color:var(--text-navy);">
                                                        <fmt:formatNumber value="${inv.totalAmount}" type="number"
                                                            maxFractionDigits="0" /> đ
                                                    </span></td>
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${inv.status eq 'Pending'}"><span
                                                                class="inv-badge inv-pending">CHƯA TT</span></c:when>
                                                        <c:when test="${inv.status eq 'Paid'}"><span
                                                                class="inv-badge inv-paid">ĐÃ TT</span></c:when>
                                                        <c:when test="${inv.status eq 'Refunding'}"><span
                                                                class="inv-badge inv-refunding">CHỜ HOÀN</span></c:when>
                                                        <c:otherwise><span class="inv-badge inv-cancelled">ĐÃ HUỶ</span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </td>
                                                <td>
                                ...
                                <div class="table-pagination-bar">
                                    <div class="pagination-info">
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                Hiển thị ${(page-1)*pageSize + 1}-${page*pageSize gt totalItems ?
                                                totalItems : page*pageSize} trong số ${totalItems} hóa đơn
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
