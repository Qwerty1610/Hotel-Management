...
/* ============================================================
   5. BOOKING_ROOM AND BOOKING_SERVICE TABLES
   ============================================================ */
...
IF OBJECT_ID(N'dbo.BookingService', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.BookingService (
        ...
        CONSTRAINT FK_BookingService_Booking FOREIGN KEY (booking_id) REFERENCES dbo.Booking(booking_id) ON DELETE CASCADE,
        CONSTRAINT FK_BookingService_Service FOREIGN KEY (service_id) REFERENCES dbo.HotelService(service_id) ON DELETE CASCADE
    );
END
GO

/* Assign room for checked in booking mock data */
IF NOT EXISTS (SELECT 1 FROM dbo.BookingRoom WHERE booking_id = (SELECT TOP 1 booking_id FROM dbo.Booking WHERE customer_name = N'Ngô Thị F' AND status = N'CheckedIn'))
BEGIN
    INSERT INTO dbo.BookingRoom (booking_id, room_id)
    VALUES (
        (SELECT TOP 1 booking_id FROM dbo.Booking WHERE customer_name = N'Ngô Thị F' AND status = N'CheckedIn'),
        (SELECT TOP 1 room_id FROM dbo.Room WHERE room_number = N'101')
    );
END
GO
